...
if 'meituanjingweidu' in os.environ:
    meituanjingweidu = os.environ.get("meituanjingweidu")
else:
    meituanjingweidu = ['']
    print('无meituan经纬度变量,可以去https://lbs.amap.com/tools/picker 获取')
...
def grab_red_packet(token):
    ...
    "User-Agent": "Mozilla/5.0 (Linux; Android 14;) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/96.0.4664.104 Mobile Safari/537.36",
...
#    print(json.dumps(mtgsig))
    ...
    if response.status_code == 200:
        ...
        print("请求失败(如果急需使用,可以在手机浏览器打开https://iw.mk/mt 获取优惠券):", response.status_code, response.text)
